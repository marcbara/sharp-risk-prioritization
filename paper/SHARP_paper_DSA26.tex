...
\section{Simulation-Based Evaluation Design}
...
\textbf{Metrics enabled by the full design.} The controlled ground truth enables precise measurement of \emph{critical risk retention rate} (planted critical risks correctly classified as FLAG/ESCALATE), \emph{volume reduction} (percentage classified as MONITOR), \emph{cluster detection accuracy} (identification of planted dangerous combinations), \emph{initial false negative rate} (critical risks wrongly sent to MONITOR on first pass), \emph{recovery rate} (false negatives correctly reclassified after subsequent events), and \emph{time to recovery} (events required for self-correction).

\textbf{Metrics reported for the static slice.} The static pass exercised in this paper reports the subset that does not require streamed events: critical retention rate, initial false-negative rate, volume reduction, per-tier recall, exact-match accuracy, mean confidence by assigned tier, and end-to-end API cost. Cluster detection accuracy, recovery rate, and time to recovery require the multi-event loop and are deferred to follow-on work.
...
\textbf{Confidence as a diagnostic signal.} Mean AI-reported confidence ordered as expected by design: ESCALATE 0.95, MONITOR 0.79, FLAG 0.75. The FLAG tier carrying the lowest mean confidence is consistent with the intended role of FLAG as the ambiguous middle band, and suggests that self-reported confidence may be useful as a trigger for human review (Layer~1, Section~\ref{sec:discussion}). Calibration and run-to-run stability of this signal remain to be evaluated.
...
Bara~Iniesta, M.: Augmenting human decision-making in risk management: An {LLM}-based framework for enhancing project risk register quality. In: International Summer Conference on Intelligent Systems \& Decision Making: Human Insights in the Era of AI. University of Catania, Italy (2025)
